...
bioDat$monthYear <- as.yearmon(bioDat$moYr, "%b-%y")
...
abv <- ggplot(marsh.biomass[marsh.biomass$region %in% "LUMCON", ], aes(y = above, x = monthYear)) + geom_pointrange(aes(ymax = above + above.se, ymin = above - above.se)) + theme_classic() + facet_grid(site ~ .)

### NAPP
# "dead" column in napp is the sum of standing dead and litter
napp2 <- nappCalc(napp, summarize = "TRUE", EOS = "TRUE", EOS_window = 0)
