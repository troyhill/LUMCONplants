...
napp.melt <- melt(napp[grepl(x = napp$site, pattern = "LUM"), c(2, NAPP_columns)], id.vars = c("napp.smalley", "year"))
...
napp.correlations <- ggplot(napp.melt, aes(x = napp.smalley, y = value, col = variable)) +
  ...
  theme_classic() + theme(legend.title = element_blank()) + 
  geom_point(aes(shape = as.factor(year))) + ylim(0, 3800) + xlim(0, 3400) + 
...
rsqr.salinity <- round(summary(lm(napp.smalley ~ salinity..psu., data = napp.nuts))$r.squared, 2)

rsqr.salinity.lum <- round(summary(lm(napp.smalley ~ salinity..psu., data = napp.nuts[napp.nuts$region %in% "LUMCON", ]))$r.squared, 2)


napp.salt <- ggplot(napp.nuts, aes(x = salinity..psu., y = napp.smalley)) + geom_point(aes(colour = site)) + theme_classic() + geom_smooth(method = "lm") + 
  labs(y = expression("Smalley NAPP (g "%.%m^-2%.%yr^-1~")"), x = "Creek salinity (psu)") # all data - looks nice

napp.salt.lum <- ggplot(napp.nuts[napp.nuts$region %in% "LUMCON", ], aes(x = salinity..psu., y = napp.smalley, colour = site)) + geom_point() + theme_classic() + geom_smooth(method = "lm") + 
  labs(y = expression("Smalley NAPP (g "%.%m^-2%.%yr^-1~")"), x = "Creek salinity (psu)") # weak!

### NO3 relationship
rsqr.no3 <- round(summary(lm(napp.smalley ~ dissolved.no3...no2..mm., data = napp.nuts))$r.squared, 2)

rsqr.no3.lum <- round(summary(lm(napp.smalley ~ dissolved.no3...no2..mm., data = napp.nuts[napp.nuts$region %in% "LUMCON", ]))$r.squared, 2)


napp.no3 <- ggplot(napp.nuts, aes(x = I(log(dissolved.no3...no2..mm.)), y = I(log(napp.smalley)))) + geom_point(aes(colour = site)) + theme_classic() + geom_smooth(method = "lm")  + 
  labs(y = expression("Smalley NAPP (g "%.%m^-2%.%yr^-1~")"), x = "Creekwater NO3 (uM)") # all data - looks pretty nice

napp.no3.lum <- ggplot(napp.nuts[napp.nuts$region %in% "LUMCON", ], aes(x = dissolved.no3...no2..mm., y = napp.smalley, colour = site)) + geom_point() + theme_classic() + geom_smooth(method = "lm") + 
